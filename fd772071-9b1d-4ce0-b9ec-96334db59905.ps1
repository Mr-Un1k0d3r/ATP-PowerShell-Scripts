$Source = @"
                        ...
                        [EventData]
                        public class CollectedWindowsUpdate
                        {
                              ...
                              public CollectedWindowsUpdate(string updateID, string kb, string title, string status, string category, string installationDate)
                              {
...
$collectedWindowsUpdaetsProvider =  [EtwProvider]::log
...
ForEach ($searchResult in $searchResults) {            
    $kb = $null
    $category = $null;
    $category = $($searchResult.Categories).Name;
    if($category -notlike "Windows Defender")            
    {
        if($searchResult.Title -match "KB\d{6,7}")
        {                               
            $kb = $searchResult.Title | Select-String -Pattern "KB\d{6,7}" |
            ...
            if($searchResult.ResultCode -eq 2 -or $searchResult.ResultCode -eq 3)
            {
                ...
                {            
                    2 { $installationResult = 'Succeeded' }            
                    3 { $installationResult = 'SucceededWithErrors' }                           
                }
                $installationDate= $null;
                $installationDate = Get-Date -Date $searchResult.Date;                        
                $updateId = $null;
                $updateId = $searchResult.UpdateIdentity.UpdateID;
                $title = $null;
                $title = $searchResult.Title;
                $category = $null;
                $category = $($searchResult.Categories).Name
                $collectedWindowsUpdate = new-object -TypeName "CollectedWindowsUpdate" -args $updateId, $kb, $title, $installationResult, $category, $installationDate
            
                $collectedWindowsUpdaetsProvider.Write("CollectedWindowsUpdate", $collectedWindowsUpdate)               
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDY+pdG46ZrCNRd
# QDf9TjhoENsQUVxo02Yw4fDDtHI6eaCCDZcwggYVMIID/aADAgECAhMzAAADEBr/
...
# LwYJKoZIhvcNAQkEMSIEIB/Km7GaBymm0scRHqeBX9BRH5iaBLeuE/0pKYLRbrzp
# MEIGCisGAQQBgjcCAQwxNDAyoBSAEgBNAGkAYwByAG8AcwBvAGYAdKEagBhodHRw
# Oi8vd3d3Lm1pY3Jvc29mdC5jb20wDQYJKoZIhvcNAQEBBQAEggEAPqnBByayW5eb
# ILykUpXQW53Dk2D/MZSdI2TNk41lMEYeaSHauazQW3XpRwu+l5m9N3Zu5tfgBCyx
# KExxXaT0kbe/PoTCwQz8F+D0OEmmYSe7TNw3ORnixUDVeccYzOOzpoARC41bpUlS
# wPxGa6Fs0jMK4jLMfGiXEFnK7SDLGotMWCwGCMN+LEaX1HLKf0G73pWIv42mIY1B
# UpV7HRveJTz/5NPFQNgkapLqR5O9ONrT6WINiiGhqkl35PqJXZ3EKm7GLzB61Q/Y
# D/FEIbN03kt4RACw8f4UfzzRd+9cGEkzFPfjqekH7F62bU1/ulISURkXGycfZbHb
# VDGVWbkZwqGCFxYwghcSBgorBgEEAYI3AwMBMYIXAjCCFv4GCSqGSIb3DQEHAqCC
# Fu8wghbrAgEDMQ8wDQYJYIZIAWUDBAIBBQAwggFZBgsqhkiG9w0BCRABBKCCAUgE
# ggFEMIIBQAIBAQYKKwYBBAGEWQoDATAxMA0GCWCGSAFlAwQCAQUABCDH4bqISJzo
# htETdIMXLMnUFWBHXsXDsjWjCdqjr4EsmQIGYxIJsT8EGBMyMDIyMDkyMDA5NTcx
# OS40ODFaMASAAgH0oIHYpIHVMIHSMQswCQYDVQQGEwJVUzETMBEGA1UECBMKV2Fz
...
# MC8GCSqGSIb3DQEJBDEiBCDiKRxzzgvsBieeqIMVGFkr1gU/ATtlt3NdovmpsDQj
# STCB+gYLKoZIhvcNAQkQAi8xgeowgecwgeQwgb0EIGbp3u2sBjdGhIL4z+ycjtzS
...
# 71dTb23p9he3LnoP000wDQYJKoZIhvcNAQELBQAEggIAMgufJ8jIlgHtmWRyHIUT
# gwTADIXBO+OfVBsFRT2pGeD6Ri5mjY75sCG4wXLS9fhYFDjEyWzX6BZSIGslsn+W
# pFspi6l/o4vPjQI/+cHLny6jJfFTtekycwMe33uZaIuYfpV5d4t/OUxDszf89mXP
# 5MiP4FroKY71W2AHWVjk/U139dBMM3K+Xr58tphWzAjVX5YEvvHR5+bkHcBZ+Xbx
# sJ/cjXHBFHDMtwBiDTmn0VCfViaR5Iew8o0fg+pptVEtFOkVY9OkrZE/+0an+KHS
# O5hHSZjhsq1p90xWEvd3CmCIGtiPcxuNaZmHafVP7t+mDmZrz9CZuSV+W5azday8
# kmIe0EAmPtqA/uA3B1Ql7dOfNhtWoUjO6DWUoILPIqiku6CmDcQFk1cXzdwg9Vz1
# 4t4DfhF+nwXDdXA9+1dtHlXrc5b2Je3O6rSK52lXH8qIQN8xStfxTNiPdzclDUMp
# LIyTwNybzHdD/FXzMAS2U06uIZnP+yIuml8+uNm8YoB7FvsIYHeJMLqLLnatDYc8
# PLJISmUTnKLiLnwnjJQa8+FKn9z5e8n8WKhAg5nEUJg9kcimRK6IZZnIbOGyY8t8
# TNyBTUSndWAQerFo0TKFReo6QFLEmPT49XuQkxJMz3FsnmLMUjiikhFd5LRZm0iV
# TjQR7k+k3MDBZ3lb8PQG0WA=
