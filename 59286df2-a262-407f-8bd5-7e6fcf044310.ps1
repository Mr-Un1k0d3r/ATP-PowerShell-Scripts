$Source = @"
                        ...
                        public class CollectedService
                        {
                                ...
                                public CollectedService(string Id, string name, string displayName, string description, string serviceType, string startMode, string path, string parameters, string executingAccount,string sddl, bool isPathSuspicious, bool isPathUnquoted, bool hasCachedPlainPassword)
                                {
									this.ID = Id;
                                    this.Name = name;
...
if(((($Service.Type -band 16 ) -eq 16 ) -or (($Service.Type -band 32 ) -eq 32))-and (($Service.Type -band 64 ) -ne 64)){ # filter only Win32 own processes and Win32 shared processes
    ...
    if($executingAccount -like 'NT SERVICE\*')  
    {
    ...
    $collectedService = New-Object CollectedService($serviceName, $serviceName, $displayName, $description, $serviceType, $startMode, $pathName, $parameters, $executingAccount, $sddl, $isPathSuspicious, $isPathUnquoted, $hasCachedPlainPassword)                 
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAI30Zq4y6MoKdo
# CoQvHTz1rpzFxau27B+3t02LOKaN+6CCDZcwggYVMIID/aADAgECAhMzAAADEBr/
...
# LwYJKoZIhvcNAQkEMSIEIPNnBD+rRec2Wq+WqywDE4GfiN0+LD1dDsbvDGet0brM
# MEIGCisGAQQBgjcCAQwxNDAyoBSAEgBNAGkAYwByAG8AcwBvAGYAdKEagBhodHRw
# Oi8vd3d3Lm1pY3Jvc29mdC5jb20wDQYJKoZIhvcNAQEBBQAEggEAl8O0jDK7ZnpJ
# xZ0RkU/5SGMa9a5+IdP6TQwCmFwGwaUOTaGa+0iMd75bDjpOL+DgN+LdOO6Ku2v+
# olYAOAS+rZYyL0qlLSnnziD1qafU5AvBLQIvzyFGton5PobrauEHhD6rKTHCzY4G
# ZW9308hfVG2zutHDHP9eTDLC4NkCKBNkIE5axPHhSgqrr0FpKDYIsYartojjhjyL
# fue0ZN8X3vI2PM4GSQLxZERKHvbCLV4tXqzOmXHo2ipzxvrQ2wU/MfqPoewzJbmz
# 3Ldtxok0lK1UpbvTa5Yod5qhYXhkDsqmTu6lbpPKibEzp8EUezVlQ65+k/6008VH
# 5OZGBmkMOaGCFxYwghcSBgorBgEEAYI3AwMBMYIXAjCCFv4GCSqGSIb3DQEHAqCC
# Fu8wghbrAgEDMQ8wDQYJYIZIAWUDBAIBBQAwggFZBgsqhkiG9w0BCRABBKCCAUgE
# ggFEMIIBQAIBAQYKKwYBBAGEWQoDATAxMA0GCWCGSAFlAwQCAQUABCC5RM1a8Mgy
# gaJgIJh3xdHEwhqD2jGr+H4qgCah3tKBCAIGYxIL5SyCGBMyMDIyMDkyMDA5NTcy
# NS43MTRaMASAAgH0oIHYpIHVMIHSMQswCQYDVQQGEwJVUzETMBEGA1UECBMKV2Fz
...
# MC8GCSqGSIb3DQEJBDEiBCCV85mAMQUT20JYn/FQGJIHw05KyjVDsoOCKTgir75/
# bzCB+gYLKoZIhvcNAQkQAi8xgeowgecwgeQwgb0EIPS94Kt130q+fvO/fzD4MbWQ
...
# 9vaozehuIIRSeDWPQDgwDQYJKoZIhvcNAQELBQAEggIAla85tSpf2RXR41OCydeY
# PHmBEKLLzmj7xZqQ4z3NlRwizBQeAinqgOCIIOb85xesLjB0hRkrZW/6HATGAS8D
# mB7WhPrtTgezkEG7dz96Qpkr6n9/7glBobSp0am/P83u+952zeaYC9rtc82YkT5t
# /GoVqwkSQonleRfN5PNdOE0nD528L0RCzn0LkvZ+XjhOGHzOL6bOIMYlWhDlWBfB
# 3kWPxpSl4a0+gCpoGQVF59mADn+evZ1bsjs5Kre+FIm/vSM7mldypX0Gjl28Lo94
# pwOBO+kNcVAi+iY6givUr2Vnzmq9UfD1lCu3zqf5+A81eHPmEClE7tZMa3W2jHC0
# neAdI9xklzphZd6MYR8+bNb62t4uYcnpzO1lOuyGZQ2R4oLJsxHWHVfgt7OH/5bo
# wOx9NUxjdjKGi7tweOVRVz8IzYrYkQuzyDTDVpAz0KmOoaa7EOow9wIL1MEAQiKw
# 2357ydxO59/p7dYqTosFhNufXWQhPuManYtTxvgZw82ELQGkyab8sO7nf4cz8Gk7
# kEO3+AdMnhMWxtw+psFDN3Hlawx553K1yxEWbpWofeOWSPntOxMzcaq/OHfxEDGV
# sphUY1pXI+S1CxHBYWJ0mkM42YFsZMxB8TCyUD1vd1dmn8sdKlZUy/aTDyyDAZ0m
# x4QmvzbBBvk0snUa9nuboaY=
